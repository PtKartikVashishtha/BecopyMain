...
const Contributors = () => {
  ...
  const { items } = useAppSelector((state) => state.contributors);
  ...
  useEffect(() => {
    const checkOverflow = () => {
      ...
      if (!itemEl || filteredContributors.length === 0) return;

      const singleItemWidth = itemEl.offsetWidth + ITEM_GAP;
      setItemWidth(singleItemWidth);

      const contentWidth = singleItemWidth * filteredContributors.length;
  ...
  useEffect(() => {
    ...
    autoScrollRef.current = setInterval(() => {
      const containerWidth = containerRef.current?.offsetWidth ?? 0;
      const contentWidth = itemWidth * filteredContributors.length;
  ...
  }, [shouldAutoScroll, itemWidth, filteredContributors.length, offsetX, controls]);
